<?php
/*
 * Webflow > drupal Generator
 */
$zip = new ZipArchive;
	if ($zip->open('fit.zip') === TRUE) {
	    $zip->extractTo('html/');
	    $zip->close();
	...
	$files = array();
		foreach($htmlfiles as &$file){						
			...
			if (!file_exists('components/')) {
			    ...
			    // echo "$pageBundle \r\n  ";
			    // echo '<textarea style="margin: 0px;     height: 632px; width: 1520px;">'.$page."</textarea>";

			// $twig = file_get_contents('template/template.twig');
			foreach ($extracted_info as &$info) {
			    ...
			    // echo "$yaml_filename \r\n  ";
			    // echo '<textarea style="margin: 0px;     height: 632px; width: 1520px;">'.$yaml."</textarea>";
			}
}
